...
const Pipeline = require('./src/pipeline'); // 引入你写好的 Pipeline 类
...
// 2. 将大模型生成的 Markdown 格式脚本转化为适合邮件渲染的 HTML
...
async function formatDeepVideoEmail(articles) {
  ...
  let htmlContent = `
    <div style="font-family: Microsoft YaHei, Arial, sans-serif; max-width: 900px; margin: 0 auto; color: #222; line-height: 1.8;">
      <div style="background: linear-gradient(135deg, #111827, #1e3a8a); color: white; padding: 25px; border-radius: 10px; margin-bottom: 25px;">
        <h1 style="margin: 0; font-size: 24px;">🎬 DeepSeek 实时生成 - 5-8分钟深度视频分镜脚本</h1>
  ...
  for (let index = 0; index < articles.length; index++) {
    ...
    console.log(`[${index + 1}/${articles.length}] 正在调用 DeepSeek API 生成脚本: ${art.title}`);

    let generatedScript = '';
    try {
      // 🌟 核心变化：不再使用拼接套话，而是真正异步调用 DeepSeek 大模型！
      generatedScript = await pipeline.generateScript({
        title: art.title,
        content: art.description || art.content || ''
      });
    } catch (err) {
      console.error(`新闻 ${index + 1} 脚本生成失败:`, err.message);
      generatedScript = `⚠️ AI 生成失败，请检查 API Key 额度或网络：${err.message}`;
    }
...
async function sendEmail(htmlContent) {
  ...
  await transporter.sendMail({
    ...
    to: EMAIL_RECIPIENT,
    subject: `【DeepSeek 深度闭环脚本】${new Date().toLocaleDateString('zh-CN')} 全球热点特辑`,
    html: htmlContent
  });
